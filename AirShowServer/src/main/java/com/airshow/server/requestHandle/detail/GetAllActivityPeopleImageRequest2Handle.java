...
public class GetAllActivityPeopleImageRequest2Handle extends RequestHandle {
    private static Logger logger = Logger.getLogger(GetAllActivityPeopleImageRequest2Handle.class);
	public GetAllActivityPeopleImageRequest2Handle(ChannelHandlerContext ctx,
                                                   String contentStr) {
	...
	public String getHandleResult() {
        ...
        ResultSet rs=null;
		try {
			String sql = "select d.*,i.path as image_path from activity_people_image_2 d left join image i on d.image_id = i.id where d.pid = ? limit " + (getAllActivityPeopleImageRequest.getPage() -1)*10 + ",10";
            ...
            sql = "select count(*) from activity_people_image_2 where pid = ?";
            ps = connection.prepareStatement(sql);
            ps.setInt(1,getAllActivityPeopleImageRequest.getPid());
            rs = ps.executeQuery();
