...
public class GetAdByTypeRequestHandle extends RequestHandle {
    private static Logger logger = Logger.getLogger(GetAdByTypeRequestHandle.class);
	public GetAdByTypeRequestHandle(ChannelHandlerContext ctx,
			String contentStr) {
	...
	public String getHandleResult() {
		Gson gson = new Gson();
		GetAdByTypeRequest getAdByTypeRequest;
		try {
			getAdByTypeRequest = gson.fromJson(contentStr,GetAdByTypeRequest.class);
		} catch (Exception e) {
        ...
        ResultSet rs=null;
		try {
			String sql = "select * from ad,image where ad.image_id = image.id and ad.type = ?";
			ps = connection.prepareStatement(sql);
			ps.setInt(1, getAdByTypeRequest.getType());
			rs = ps.executeQuery();
