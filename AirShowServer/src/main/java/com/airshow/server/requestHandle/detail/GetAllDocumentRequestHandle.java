...
public class GetAllDocumentRequestHandle extends RequestHandle {
    private static Logger logger = Logger.getLogger(GetAllDocumentRequestHandle.class);
	public GetAllDocumentRequestHandle(ChannelHandlerContext ctx,
                                       String contentStr) {
	...
	public String getHandleResult() {
        Gson gson = new Gson();
        GetAllDocumentRequest getAllDocumentRequest;
        try {
            getAllDocumentRequest = gson.fromJson(contentStr,GetAllDocumentRequest.class);
        } catch (Exception e) {
        ...
        ResultSet rs=null;
		try {
			String sql = "select d.*,i.path as image_path from document d left join image i on d.image_id = i.id ";
